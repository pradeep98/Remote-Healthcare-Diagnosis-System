...
datafile="./fram11.data"
headers=['age','diabetes','sysBP','diaBP','glucose','TenYearCHD']
...
train_data = pd.DataFrame({'age':data_nm['age'].iloc[0:train_size], 'diabetes':data_nm['diabetes'].iloc[0:train_size],'sysBP':data_nm['sysBP'].iloc[0:train_size],'diaBP':data_nm['diaBP'].iloc[0:train_size],'glucose':data_nm['glucose'].iloc[0:train_size]},index=data_nm.index[0:train_size])

train_target = pd.DataFrame({'TenYearCHD':data_nm['TenYearCHD'].iloc[0:train_size]},index = data_nm.index[0:train_size])

test_data = pd.DataFrame({'age':data_nm['age'].iloc[train_size:rows+1], 'diabetes':data_nm['diabetes'].iloc[train_size:rows+1],'sysBP':data_nm['sysBP'].iloc[train_size:rows+1],'diaBP':data_nm['diaBP'].iloc[train_size:rows+1],'glucose':data_nm['glucose'].iloc[train_size:rows+1]},index=data_nm.index[train_size:rows+1])
...
model=Sequential()
model.add(Dense(128,input_dim=5,init="glorot_uniform",activation="relu",kernel_regularizer=regularizers.l2(0.01),activity_regularizer=regularizers.l1(0.01)))
...
'diabetes':0,'sysBP':70,'diaBP':95,'glucose':103},index=[0])
