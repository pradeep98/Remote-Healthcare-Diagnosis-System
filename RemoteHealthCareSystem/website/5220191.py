...
data=[]
data = pd.read_csv("/home/aryan/Desktop/heart/fram.csv",delimiter=",")
data = data.dropna()
scaler = MinMaxScaler()
...
datafile="./fram.data"
headers=['age','diabetes','sysBP','diaBP','BMI','heartRate','glucose','TenYearCHD']
...
train_data = pd.DataFrame({'age':data_nm['age'].iloc[0:train_size], 'diabetes':data_nm['diabetes'].iloc[0:train_size],'sysBP':data_nm['sysBP'].iloc[0:train_size],'diaBP':data_nm['diaBP'].iloc[0:train_size],'BMI':data_nm['BMI'].iloc[0:train_size],'heartRate':data_nm['heartRate'].iloc[0:train_size],'glucose':data_nm['glucose'].iloc[0:train_size]},index=data_nm.index[0:train_size])

train_target = pd.DataFrame({'TenYearCHD':data_nm['TenYearCHD'].iloc[0:train_size]},index = data_nm.index[0:train_size])

test_data = pd.DataFrame({'age':data_nm['age'].iloc[train_size:rows+1], 'diabetes':data_nm['diabetes'].iloc[train_size:rows+1],'sysBP':data_nm['sysBP'].iloc[train_size:rows+1],'diaBP':data_nm['diaBP'].iloc[train_size:rows+1],'BMI':data_nm['BMI'].iloc[train_size:rows+1],'heartRate':data_nm['heartRate'].iloc[train_size:rows+1],'glucose':data_nm['glucose'].iloc[train_size:rows+1]},index=data_nm.index[train_size:rows+1])
...
model=Sequential()
model.add(Dense(128,input_dim=7,init="glorot_uniform",activation="relu",kernel_regularizer=regularizers.l2(0.01),activity_regularizer=regularizers.l1(0.01)))
...
acc_downsampled=model.evaluate(x=np.array(test_data),y=np.array(test_target),verbose=2)

print("Downsampled error and accuracy ",acc_downsampled)
# In[43]:

x = pd.DataFrame({'age':61,
'diabetes':0,'sysBP':70,'diaBP':95,'BMI':28.58,'heartRate':65,'glucose':103},index=[0])
